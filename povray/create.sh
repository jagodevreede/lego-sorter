...
function random_background_color {
  R="0.$(awk -v min=940 -v max=999 -v seed=$RANDOM 'BEGIN{srand(seed); print int(min+rand()*(max-min+1))}' | awk '{ printf("%03d\n", $1) }' )"
  G="0.$(awk -v min=940 -v max=999 -v seed=$RANDOM 'BEGIN{srand(seed); print int(min+rand()*(max-min+1))}' | awk '{ printf("%03d\n", $1) }' )"
  B="0.$(awk -v min=940 -v max=999 -v seed=$RANDOM 'BEGIN{srand(seed); print int(min+rand()*(max-min+1))}' | awk '{ printf("%03d\n", $1) }' )"
...
function create_img {
    ...
    cam_Z="$(awk -v min=11 -v max=12 -v seed=$RANDOM 'BEGIN{srand(seed); print min+rand()*(max-min+1)}')"
    ...
    povray +L${LGEO_FOLDER}/lg +L${LGEO_FOLDER} +L${LGEO_FOLDER}/ar +W${RENDER_SIZE_W} +H${RENDER_SIZE_H} -GA +WL0 $1.pov
    mv $1.png $1/$1-$2-$3-$cam_X-$cam_Y-$cam_Z.png
}
